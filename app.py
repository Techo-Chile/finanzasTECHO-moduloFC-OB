...
def main():
    ...
    if(ref_date == 'today'):
      ...
      fecha = date(int(ref_date.split('/')[0]), int(ref_date.split('/')[1]), int(ref_date.split('/')[2]))
    ...
    #Fin de la semana
    week_friday = fecha + datetime.timedelta(days=-fecha.weekday()) + timedelta(days=5)
    mont_end = calendar.month_name[week_friday.month]
    
    #3. Nomina Proveedores Operacionales semana  09 - 26 al 02 marzo
    provider('Nómina Proveedores OP Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'OP', fecha, output_folder)

    #4. Nomina Proveedores TDV semana  09 - 26 al 02 marzo
    provider('Nómina Proveedores TDV Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'TDV', fecha, output_folder)
    
    #5. Nomina Reembolsos Operacionales semana 09 - 26 al 02 marzo
    reembolsos('Nómina Reembolsos OP Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'OP', fecha, output_folder)

    #6. Nomina Reembolsos TDE semana 09 - 26 al 02 marzo
    reembolsos('Nómina Reembolsos TDE Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'TDE', fecha, output_folder)

    #7. Nomina Reembolsos TDV semana 09 - 26 al 02 marzo
    reembolsos('Nómina Reembolsos TDV Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'TDV', fecha, output_folder)

    #7. Nomina Reembolsos FNDR semana 09 - 26 al 02 marzo
    reembolsos('Nómina Reembolsos FNDR Semana {} - {} {} to {} {}.xlsx'.format(week_of_year, mont_init, week_monday, mont_end, week_friday), 'FNDR', fecha, output_folder)


def reembolsos(outputName, opType, fecha, output_folder):
    try:
        inputFile = join(output_folder  + '/WorkingFiles/', 'Flujo caja 2018.xlsx')
        ...
        lst = inputFileReader.filterDate(inputFileReader.getRowList(),
                                        datetime.datetime.combine(fecha, time(0, 0)), 4)
...
def provider(outputName, opType, fecha, output_folder):
  ...
  Args:
    ...
    try:
        ...
        dbFile = join(output_folder + '/WorkingFiles/', 'BD Transferencia Proveedores..xlsx')
        dbFileReader = read.Reader(dbFile)
        dbRows = dbFileReader.getRowList()
...
def copy_base_files(propertiesMap):
      ...
      output_folder = join(output_folder, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
      if(not os.path.exists(output_folder)):
            os.makedirs(output_folder)
            os.makedirs(output_folder + '/WorkingFiles/')
      copyfile(proveed_file, join(output_folder + '/WorkingFiles/', os.path.basename(proveed_file)))
      copyfile(flujo_caja_file, join(output_folder + '/WorkingFiles/',os.path.basename(flujo_caja_file)))
      copyfile(employees_file, join(output_folder + '/WorkingFiles/',os.path.basename(employees_file)))
